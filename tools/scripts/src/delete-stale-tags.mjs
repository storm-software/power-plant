...
const SCOPE_PREFIX = "@power-plant/";
...
try {
  ...
  const staleTags = [];
  for (const packagesDir of PACKAGES_DIRS) {
    const allTags = (await $`git tag -l`).stdout
      .trim()
      .split("\n")
      .filter(Boolean);
    const tagsByPrefix = buildTagsByPrefix(allTags);

    staleTags.push(...findStaleTags(packagesDir, tagsByPrefix));
  }
