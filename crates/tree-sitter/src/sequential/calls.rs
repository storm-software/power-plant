...
use super::super::extract::{FileResult, module_is_dir};
...
pub fn pass_calls(ctx: &mut PipelineContext<'_>, files: &[FileInfo]) -> PipelineResult<()> {
  for (i, file) in files.iter().enumerate() {
    ...
    for call in &result.calls {
      ...
      let resolution = state.registry.resolve(&call.callee_name, &module_qn, &import_map);
      ...
      .to_string();
      state.graph.insert_edge(src.id, target.id, "CALLS", &props);
    }
