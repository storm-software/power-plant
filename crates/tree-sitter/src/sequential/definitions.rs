...
use super::super::extract::{
  Channel, ChannelDirection, Definition, EnvAccess, FileResult, Import, ReadStatus, is_quarantined,
  label_is_registry_symbol, oversized_reason, quarantine_phase, read_file,
};
use super::super::fqn::{self, compute_fqn};
use super::super::graph::build_import_props;
use super::SequentialPipelineState;
use crate::{FileErrorPhase, FileInfo};

pub fn pass_definitions(ctx: &mut PipelineContext<'_>, files: &[FileInfo]) -> PipelineResult<()> {
  ...
  for (i, file) in files.iter().enumerate() {
    ...
    if is_quarantined(rel) {
      ...
      let reason =
        if phase == "hang" { "quarantined after hang" } else { "quarantined after crash" };
    ...
    let source = match read_file(&file.path) {
      ...
      Err(ReadStatus::Oversized) => {
        ...
        ctx.pipeline.add_file_error(rel, oversized_reason(size), FileErrorPhase::Oversized);
        errors += 1;
        continue;
      }
      Err(ReadStatus::OpenFail | ReadStatus::Oom) => {
        ctx.pipeline.add_file_error(rel, "read failed", FileErrorPhase::Read);
        errors += 1;
        continue;
      }
    ...
    let result = ctx.sequential.extractor().extract_file(&source, language, &project_name, rel)?;
    ...
    if owns_local_cache {
      ...
      total_imports +=
        create_import_edges(&mut ctx.sequential, &project_name, &result, rel, &namespace_map);
  ...
  if owns_local_cache {
    let namespace_map = build_namespace_map(&project_name, &ctx.sequential.result_cache, files);
    for (i, file) in files.iter().enumerate() {
      ctx.check_cancel()?;
      ...
      create_channel_edges(&mut ctx.sequential, &project_name, &result, &file.rel_path);
      create_env_configures(&mut ctx.sequential, &project_name, &result, &file.rel_path);
    }
  }
...
fn process_def(
  ...
  if node_id > 0 && label_is_registry_symbol(&def.label) {
    state.registry.add(&def.name, &def.qualified_name, &def.label);
  }
  ...
  if def.label == "Method" {
    if let Some(parent_qn) = def.parent_class.as_deref() {
      if let Some(parent) = state.graph.find_by_qn(parent_qn) {
        state.graph.insert_edge(parent.id, node_id, "DEFINES_METHOD", "{}");
      }
    }
  }
}
...
fn create_import_edges(
  ...
  let mut count = 0;
  for imp in &result.imports {
    let Some(target) = resolve_import_node(state, project_name, rel, &file_qn, imp, namespace_map)
    else {
      continue;
    };
...
fn resolve_import_node(
  ...
  let simple = imp.module_path.rsplit('.').next().unwrap_or(&imp.module_path);
...
fn create_channel_edges(
  ...
  for ch in &result.channels {
    ...
    .to_string();
    let channel_id =
      state.graph.upsert_node("Channel", &ch.channel_name, &channel_qn, "", 0, 0, &channel_props);
    ...
    state.graph.insert_edge(src_id, channel_id, edge_type, &edge_props);
  }
}
...
fn create_env_configures(
  ...
  for ea in &result.env_accesses {
    ...
    let env_id = state.graph.upsert_node("EnvVar", &ea.env_key, &env_qn, "", 0, 0, &env_props);
    let Some(src_id) = env_source(state, project_name, ea, rel, &file_qn) else {
      continue;
    };
    if src_id != env_id {
      state.graph.insert_edge(src_id, env_id, "CONFIGURES", r#"{"strategy":"env_access"}"#);
      count += 1;
    }
  }
  count
...
fn env_source(
  ...
  state.graph.find_by_qn(file_qn).map(|node| node.id).or_else(|| {
