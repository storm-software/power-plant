...
impl GraphBuffer {
  ...
  pub fn find_by_simple_name(&self, simple: &str, exclude_qn: &str) -> Option<GraphNode> {
    self.nodes_by_qn.values().find(|n| n.name == simple && n.qualified_name != exclude_qn).cloned()
  }

  pub fn find_by_label(&self, label: &str) -> Vec<&GraphNode> {
    self.nodes_by_qn.values().filter(|n| n.label == label).collect()
  }
  ...
  pub fn upsert_node(
    ...
    self.nodes_by_qn.insert(qualified_name.to_string(), node.clone());
  ...
  pub fn insert_edge(
    ...
    if self
      .edges
      .iter()
      .any(|e| e.source_id == source_id && e.target_id == target_id && e.edge_type == edge_type)
    {
      return;
    }
  ...
  pub fn delete_by_file(&mut self, rel_path: &str) {
    let doomed: Vec<i64> =
      self.nodes_by_qn.values().filter(|n| n.file_path == rel_path).map(|n| n.id).collect();
    self.nodes_by_qn.retain(|_, n| !doomed.contains(&n.id));
    for id in &doomed {
      self.nodes_by_id.remove(id);
    }
    self.edges.retain(|e| !doomed.contains(&e.source_id) && !doomed.contains(&e.target_id));
  }
}

pub(crate) fn is_dir_container(node: &GraphNode) -> bool {
  matches!(node.label.as_str(), "Folder" | "Project")
}

pub(crate) fn build_import_props(local_name: Option<&str>) -> String {
