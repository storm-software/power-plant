...
fn metadata_value_is_dlt_sourced(value: &serde_json::Value) -> bool {
  value
    ...
    .unwrap_or(false)
    || value.get("data_item_external_metadata").map(metadata_value_is_dlt_sourced).unwrap_or(false)
}
...
pub fn classify_documents(data_items: &[Data]) -> Vec<Document> {
  data_items
    ...
    .filter_map(|data| {
      ...
      base.id = data.id.to_string(); // use Data's deterministic ID
        ...
        && let Some(node_set_array) = meta_val.get("node_set").and_then(|v| v.as_array())
      {
        ...
        let node_set_values: Vec<serde_json::Value> = node_set_array
          ...
          .filter_map(|v| {
            let name = v.as_str()?;
            let key = format!("NodeSet:{name}").to_lowercase().replace(' ', "_").replace('\'', "");
...
mod tests {
  ...
  fn classifies_text_plain() {
    ...
    assert_eq!(docs[0].base.id, data[0].id.to_string());
    assert_eq!(docs[0].base.data_type, "TextDocument");
    assert_eq!(docs[0].base.get_metadata("index_fields"), Some(&serde_json::json!(["name"])));
  }
  ...
  fn classifies_image_extensions() {
    for ext in &[
      "png", "dwg", "xcf", "jpg", "jpx", "apng", "gif", "webp", "cr2", "tif", "bmp", "jxr", "psd",
      "ico", "heic", "avif",
    ] {
      let data = vec![make_data(&format!("image/{ext}"), ext)];
      let docs = classify_documents(&data);
      assert_eq!(docs.len(), 1, "failed for extension: {ext}");
      assert_eq!(docs[0].document_type, "image", "failed for extension: {ext}");
      assert_eq!(docs[0].base.data_type, "ImageDocument", "failed for extension: {ext}");
    }
  }

  #[test]
  fn classifies_audio_extensions() {
    for ext in &["aac", "mid", "mp3", "m4a", "ogg", "flac", "wav", "amr", "aiff"] {
      let data = vec![make_data(&format!("audio/{ext}"), ext)];
      let docs = classify_documents(&data);
      assert_eq!(docs.len(), 1, "failed for extension: {ext}");
      assert_eq!(docs[0].document_type, "audio", "failed for extension: {ext}");
      assert_eq!(docs[0].base.data_type, "AudioDocument", "failed for extension: {ext}");
    }
  }
  ...
  fn classifies_unstructured_extensions() {
    for ext in &["docx", "doc", "odt", "xls", "xlsx", "ppt", "pptx", "odp", "ods"] {
      ...
      let docs = classify_documents(&data);
      assert_eq!(docs.len(), 1, "failed for extension: {ext}");
      assert_eq!(docs[0].document_type, "unstructured", "failed for extension: {ext}");
      assert_eq!(docs[0].base.data_type, "UnstructuredDocument", "failed for extension: {ext}");
    }
  }

  #[test]
  fn classifies_html_extensions() {
    for ext in &["html", "htm"] {
      let data = vec![make_data("text/html", ext)];
      let docs = classify_documents(&data);
      assert_eq!(docs.len(), 1, "failed for extension: {ext}");
      assert_eq!(docs[0].document_type, "html", "failed for extension: {ext}");
      // Cross-SDK parity: Python's BeautifulSoupLoader produces a
      // TextDocument, so HTML documents carry the TextDocument data_type.
      assert_eq!(docs[0].base.data_type, "TextDocument", "failed for extension: {ext}");
    }
  }
  ...
  fn classifies_dlt_sourced_data() {
    let data = vec![make_data_with_metadata("text/plain", "txt", r#"{"source": "dlt"}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
  ...
  fn dlt_detection_with_unknown_extension() {
    ...
    let data =
      vec![make_data_with_metadata("application/octet-stream", "xyz", r#"{"source": "dlt"}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
    assert_eq!(docs[0].document_type, "dlt_row");
  }
  ...
  fn non_dlt_metadata_does_not_affect_classification() {
    let data = vec![make_data_with_metadata("text/plain", "txt", r#"{"source": "other"}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
    assert_eq!(docs[0].document_type, "text");
  }
  ...
  fn formats_external_metadata_as_pretty_json() {
    let data =
      vec![make_data_with_metadata("text/plain", "txt", r#"{"key":"value","nested":{"a":1}}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
  ...
  fn document_has_index_fields_metadata() {
    let data = vec![make_data("text/plain", "txt")];
    let docs = classify_documents(&data);
    assert_eq!(docs[0].base.get_metadata("index_fields"), Some(&json!(["name"])));
  }
  ...
  fn node_set_populates_belongs_to_set_and_source_node_set() {
    let data =
      vec![make_data_with_metadata("text/plain", "txt", r#"{"node_set": ["setA", "setB"]}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
  ...
  fn node_set_single_entry() {
    let data = vec![make_data_with_metadata("text/plain", "txt", r#"{"node_set": ["only_one"]}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
  ...
  fn no_node_set_key_leaves_belongs_to_set_unset() {
    let data = vec![make_data_with_metadata("text/plain", "txt", r#"{"other_key": "value"}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
  ...
  fn node_set_not_array_leaves_belongs_to_set_unset() {
    let data =
      vec![make_data_with_metadata("text/plain", "txt", r#"{"node_set": "not_an_array"}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
    assert!(docs[0].base.belongs_to_set.is_none());
    assert!(docs[0].base.source_node_set.is_none());
  ...
  fn node_set_empty_array_leaves_belongs_to_set_unset() {
    let data = vec![make_data_with_metadata("text/plain", "txt", r#"{"node_set": []}"#)];
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
  ...
  fn document_implements_has_datapoint() {
    ...
    let docs = classify_documents(&data);
    assert_eq!(docs.len(), 1);
    let dp_id = docs[0].base.id.clone();
