...
mod tests {
  ...
  fn test_data_point_with_dataset() {
    ...
    assert_eq!(dp.belongs_to_set, Some(vec![serde_json::json!(dataset_id.to_string())]));
  }
  ...
  fn test_metadata_operations() {
    ...
    assert_eq!(dp.get_metadata("index_fields"), Some(&serde_json::json!(["name"])));
  }
  ...
  fn source_content_hash_round_trips_when_set_and_omitted_when_none() {
    let mut dp = DataPoint::new("Entity", None);
    assert!(
      !serde_json::to_string(&dp).unwrap().contains("source_content_hash"),
