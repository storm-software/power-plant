...
use super::super::extract::{module_is_dir, read_file, FileResult};
use super::super::fqn::{compute_fqn, module_dir_fqn};
...
fn load_result(
    ...
    let extracted = state.extractor().extract_file(
        &data,
        file.language,
        project,
        &file.rel_path,
...
fn resolve_usage_edges(
    ...
    for usage in &result.usages {
        ...
        let resolution = state
            .registry
            .resolve(&usage.type_name, module_qn, import_map);
        let Some(src_id) =
            source_node_id(state, project_name, usage.enclosing_func_qn.as_deref(), rel)
        else {
            continue;
        };
        ...
        state
            .graph
            .insert_edge(src_id, tgt.id, "USAGE", &props);
    }
}
...
fn resolve_throw_edges(
    ...
    for throw_site in &result.throws {
        ...
        let resolution = state
            .registry
            .resolve(&throw_site.exception_type, module_qn, import_map);
        let Some(src_id) = source_node_id(
            state,
            project_name,
            throw_site.enclosing_func_qn.as_deref(),
            rel,
        ) else {
            continue;
        };
...
fn resolve_rw_edges(
    ...
    for rw in &result.rw {
        ...
        let resolution = state
            .registry
            .resolve(&rw.var_name, module_qn, import_map);
        let Some(src_id) =
            source_node_id(state, project_name, rw.enclosing_func_qn.as_deref(), rel)
        else {
            continue;
        };
        let Some(tgt) = state.graph.find_by_qn(&resolution.qualified_name) else {
...
fn source_node_id(
    ...
    if let Some(qn) = enclosing.filter(|s| !s.is_empty()) {
        if let Some(node) = state.graph.find_by_qn(qn) {
            if !super::super::graph::is_dir_container(node) {
                return Some(node.id);
            }
        }
    }
