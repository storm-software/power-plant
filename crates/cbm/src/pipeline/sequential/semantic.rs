...
fn process_def_edges(
    ...
    def: &super::super::extract::Definition,
    module_qn: &str,
...
fn resolve_impl_traits(
    ...
    for imp in &result.impl_traits {
        let trait_qn = if state.registry.exists(&imp.trait_qn) {
            ...
            state
                .registry
                .resolve(&imp.trait_qn, module_qn, import_map)
                .qualified_name
        };
        let type_qn = if state.registry.exists(&imp.type_qn) {
            ...
            state
                .registry
                .resolve(&imp.type_qn, module_qn, import_map)
                .qualified_name
        };
        let (Some(src), Some(tgt)) = (
            state.graph.find_by_qn(&type_qn).cloned(),
            state.graph.find_by_qn(&trait_qn).cloned(),
        ) else {
            continue;
        };
