...
impl<'a> IncrementalRun<'a> {
    pub fn classify(&self) -> IncrementalClassification {
        let (is_changed, n_changed, n_unchanged) =
            classify_files(self.files, self.stored_hashes);
        let (deleted, mode_skipped) = find_deleted_and_mode_skipped(
...
mod tests {
    ...
    fn temp_file(rel: &str, contents: &str) -> (PathBuf, FileInfo) {
        ...
        (
            ...
            FileInfo {
                ...
                language: super::extract::infer_language(rel),
            },
        )
    }
    ...
    fn mode_skipped_file_is_preserved_not_deleted() {
        ...
        let (deleted, mode_skipped) =
            find_deleted_and_mode_skipped(&_base, &[], &stored);
    ...
    fn inbound_edge_capture_skips_recomputed_types() {
        ...
        changed.insert("b.rs", ());
        assert!(!should_capture_inbound_edge(
            "SIMILAR_TO",
            "a.rs",
            "b.rs",
            &changed
