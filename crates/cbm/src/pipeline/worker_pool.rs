...
/// Options for parallel dispatch. Mirrors `cbm_parallel_for_opts_t`.
...
impl Default for ParallelForOpts {
    fn default() -> Self {
        Self {
            max_workers: default_worker_count(true),
        }
    }
}
...
/// Effective worker count honoring `CBM_INDEX_SINGLE_THREAD=1`.
pub fn effective_worker_count(initial: bool) -> usize {
    if std::env::var("CBM_INDEX_SINGLE_THREAD").is_ok_and(|v| v == "1") {
        return 1;
    }
    ...
    F: Fn(usize) + Sync,
{
    ...
    let workers = if opts.max_workers == 0 {
        effective_worker_count(true)
    } else {
        opts.max_workers
    };
