...
# Power Plant - NodeJS Bindings

The **Power Plant - NodeJS Bindings** crate binds the Power Plant runtime to the [Node.js](https://nodejs.org/) runtime using [NAPI](https://napi.dev/).
...
```bash
pnpm add -D @power-plant/bindings-x86_64-unknown-linux-gnu
```
...
```bash
npm install -D @power-plant/bindings-x86_64-unknown-linux-gnu
```
...
```bash
yarn add -D @power-plant/bindings-x86_64-unknown-linux-gnu
```
...
Run `nx build native-bindings` to build the library.

## Running unit tests

Run `nx test power-plant-bindings` to execute the unit tests via [Vitest](https://vitest.dev/).
