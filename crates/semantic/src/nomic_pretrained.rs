...
//! ```
//!
//! Then point `CBM_NOMIC_DATA_DIR` at that directory, or pass the path to [`NomicPretrained::from_dir`].
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TokenVectorMismatch { tokens, vectors } => {
                write!(
                    f,
                    "token count {tokens} does not match vector count {vectors}"
                )
            }
        }
    }
...
impl NomicPretrained {
    ...
    /// Load from `CBM_NOMIC_DATA_DIR`, or return `None` when unset / invalid.
    #[must_use]
    pub fn from_env() -> Option<Self> {
        let dir = std::env::var("CBM_NOMIC_DATA_DIR").ok()?;
        Self::from_dir(dir).ok()
    }

    /// Global singleton from `CBM_NOMIC_DATA_DIR`.
    ///
    /// # Panics
    ///
...
mod tests {
    ...
    fn loads_token_vector_files() {
        ...
        std::fs::write(
            dir.path().join("code_vectors.bin"),
            sample_blob(2, DIM),
        )
        .expect("vectors");
