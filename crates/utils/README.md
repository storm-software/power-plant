...
# Power Plant - Utilities Crate

The **Power Plant - Utilities** crate provides utility types and functions used across the Power Plant runtime.
...
```bash
cargo add power-plant-utils
```

## Building

Run `nx build native-utils` to build the library.

## Running unit tests

Run `nx test power-plant-utils` to execute the unit tests via [Vitest](https://vitest.dev/).
