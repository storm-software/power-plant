...
# Power Plant - Tracing Crate

The **Power Plant - Tracing** crate provides the tracing system for the Power Plant runtime. It allows you to trace the execution of the Power Plant runtime, and provides a way to visualize the trace in a variety of ways.
...
```bash
cargo add power-plant-tracing
```

## Building

Run `nx build native-tracing` to build the library.

## Running unit tests

Run `nx test power-plant-tracing` to execute the unit tests via [Vitest](https://vitest.dev/).
