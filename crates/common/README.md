...
# Power Plant - Common Crate

The **Power Plant - Common** crate provides shared functionality and type definitions used across the Power Plant runtime.
...
```bash
cargo add power-plant-common
```

## Building

Run `nx build native-common` to build the library.

## Running unit tests

Run `nx test power-plant-common` to execute the unit tests via [Vitest](https://vitest.dev/).
