...
bitflags! {
  ...
  pub struct IssueKindSwitcher: u64 {
    ...
    const PathIsNotValidUtf8 = 1 << 34;
    const UnableToReadFileName = 1 << 35;
    const UnableToReadFile = 1 << 36;
    const UnableToReadDir = 1 << 37;
    const UnableToStripRootPrefix = 1 << 38;
  }
}
