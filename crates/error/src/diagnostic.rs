...
#[derive(Debug, Clone)]
pub struct LabelSpan(DiagnosticFileId, Range<usize>);

impl From<(DiagnosticFileId, Range<usize>)> for LabelSpan {
  fn from((id, range): (DiagnosticFileId, Range<usize>)) -> Self {
    Self(id, range)
  }
}

impl ariadne::Span for LabelSpan {
...
pub struct Diagnostic {
  ...
  pub(crate) labels: Vec<Label<LabelSpan>>,
  pub(crate) help: Option<String>,
  pub(crate) note: Option<String>,
  pub(crate) severity: Severity,
}

type AriadneReportBuilder = ReportBuilder<'static, LabelSpan>;
type AriadneReport = Report<'static, LabelSpan>;
...
impl Diagnostic {
  ...
  fn init_report_builder(&self) -> AriadneReportBuilder {
    ...
    let mut builder = AriadneReport::build(
      ...
      LabelSpan(ArcStr::default().into(), 0..0),
    )
