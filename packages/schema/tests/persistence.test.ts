...
describe("devkit/schema/src/persistence.ts", () => {
    ...
    const context = {
      ...
      fs: {
        ...
        read: vi
          .fn()
          .mockResolvedValue(
            '{"variant":"json-schema","hash":"h","schema":{"type":"string"}}'
          )
      }
    } as any;

    await expect(
      readSchemaSafe(context, { type: "string" } as any)
    ).resolves.toMatchObject({ variant: "json-schema", hash: "h" });
  });
  ...
  it("readSchema throws when schema is missing from cache", async () => {
    ...
    await expect(
      readSchema(context, { type: "string" } as any)
    ).rejects.toThrow("does not exist in the cache");
  });
});
