...
# Power Plant - Design Tokens Format Module (DTCG) Schema

A Power Plant source for reading the input specification from a [Design Tokens Format Module (DTCG)](https://www.w3.org/community/design-tokens/) standard specification schema.
...
```bash
pnpm add -D @power-plant/dtcg-schema
```
...
```bash
npm install -D @power-plant/dtcg-schema
```
...
```bash
yarn add -D @power-plant/dtcg-schema
```
...
Run `nx build dtcg-schema` to build the library.

## Running unit tests

Run `nx test dtcg-schema` to execute the unit tests via [Vitest](https://vitest.dev/).
