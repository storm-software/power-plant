...
# Power Plant - GraphQL Schema

A Power Plant source for reading the input specification from a GraphQL schema.
...
```bash
pnpm add -D @power-plant/graphql-schema
```
...
```bash
npm install -D @power-plant/graphql-schema
```
...
```bash
yarn add -D @power-plant/graphql-schema
```
