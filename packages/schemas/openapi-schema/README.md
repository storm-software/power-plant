...
# Power Plant - OpenAPI Schema

A Power Plant source for reading the input specification from an OpenAPI schema.
...
```bash
pnpm add -D @power-plant/openapi-schema
```
...
```bash
npm install -D @power-plant/openapi-schema
```
...
```bash
yarn add -D @power-plant/openapi-schema
```
...
Run `nx build openapi-schema` to build the library.

## Running unit tests

Run `nx test openapi-schema` to execute the unit tests via [Vitest](https://vitest.dev/).
