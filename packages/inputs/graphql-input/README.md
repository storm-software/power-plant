...
# Power Plant - GraphQL Input

A Power Plant input extension for reading the source specification from a [GraphQL](https://graphql.org) schema document.
...
```bash
pnpm add -D @power-plant/graphql-input
```
...
```bash
npm install -D @power-plant/graphql-input
```
...
```bash
yarn add -D @power-plant/graphql-input
```
...
Run `nx build graphql-input` to build the library.

## Running unit tests

Run `nx test graphql-input` to execute the unit tests via [Vitest](https://vitest.dev/).
