...
# Power Plant - Terrazzo Input

A Power Plant input extension for reading the source specification from a [Design Tokens Community Group (DTCG)](https://www.w3.org/community/design-tokens/) schema document and parsing it into a Design Tokens format using [Terrazzo](https://terrazzo.app).
...
```bash
pnpm add -D @power-plant/terrazzo-input
```
...
```bash
npm install -D @power-plant/terrazzo-input
```
...
```bash
yarn add -D @power-plant/terrazzo-input
```
...
Run `nx build terrazzo-input` to build the library.

## Running unit tests

Run `nx test terrazzo-input` to execute the unit tests via [Vitest](https://vitest.dev/).
