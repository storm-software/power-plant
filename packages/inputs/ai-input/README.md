...
# Power Plant - AI Model Input

A Power Plant input extension for reading the source specification from an AI model.
...
```bash
pnpm add -D @power-plant/ai-input
```
...
```bash
npm install -D @power-plant/ai-input
```
...
```bash
yarn add -D @power-plant/ai-input
```
...
Run `nx build ai-input` to build the library.

## Running unit tests

Run `nx test ai-input` to execute the unit tests via [Vitest](https://vitest.dev/).
