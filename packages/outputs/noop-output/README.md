...
# Power Plant - Noop Output

A Power Plant output that just returns the generated documents.
...
```bash
pnpm add -D @power-plant/noop-output
```
...
```bash
npm install -D @power-plant/noop-output
```
...
```bash
yarn add -D @power-plant/noop-output
```
...
Run `nx build noop-output` to build the library.

## Running unit tests

Run `nx test noop-output` to execute the unit tests via [Vitest](https://vitest.dev/).
