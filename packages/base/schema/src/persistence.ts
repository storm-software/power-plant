 ...
 */
export function getCacheDirectory(context: PersistenceContext): string {
 ...
 */
export async function writeSchema(context: PersistenceContext, schema: Schema) {
  if (!isSchema(schema)) {
    throw new Error(
  ...
  const cacheDir = getCacheDirectory(context);
  const fsMkdir = context.fs?.mkdir ?? mkdir;
  const fsWrite = context.fs?.write ?? writeFile;

  if (!existsSync(cacheDir) && !context.fs?.mkdir) {
    await mkdir(cacheDir, { recursive: true });
  } else if (context.fs?.mkdir) {
...
export async function readSchemaSafe(
  ...
  const fsExists =
    context.fs?.exists ??
    (async (path: string) => Promise.resolve(existsSync(path)));
  const fsRead = context.fs?.read ?? readFile;

  const exists = await fsExists(cacheFilePath);
  if (!exists) {
    return undefined;
  }

  const data = await fsRead(cacheFilePath);
  if (!data) {
    return undefined;
  }

  const result = JSON.parse(typeof data === "string" ? data : data.toString());
  if (!isSchema(result)) {
    return undefined;
  }
