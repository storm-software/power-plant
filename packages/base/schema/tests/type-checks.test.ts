...
describe("json schema and source type guards", () => {
  ...
  it("validates structural json-schema guard outputs", () => {
    ...
    expect(
      isJsonSchemaArray({ type: "array", items: { type: "string" } })
    ).toBe(true);
    expect(
      isJsonSchemaBigint({
        type: "integer",
    ...
    expect(isJsonSchemaDate({ type: "string", format: "date-time" })).toBe(
      true
    );
    expect(
      isJsonSchemaEnum({ type: "string", enum: ["a"], default: "a" })
    ).toBe(true);
    ...
    expect(
      isJsonSchemaNull({ type: "null", enum: [null], default: null })
    ).toBe(true);
    ...
    expect(isJsonSchemaPrimitiveUnion({ type: ["string", "number"] })).toBe(
      true
    );
    expect(
      isJsonSchemaUnion({ anyOf: [{ type: "string" }, { type: "number" }] })
    ).toBe(true);
  ...
  it("returns false for invalid inputs", () => {
    ...
    expect(
      isJsonSchemaBigint({ type: "integer", format: "int64", enum: [1] })
    ).toBe(false);
