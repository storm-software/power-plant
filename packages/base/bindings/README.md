...
**Power Plant - NodeJS Bindings** binds the Power Plant runtime to the [Node.js](https://nodejs.org/) runtime using [NAPI](https://napi.dev/).
...
```bash
pnpm add -D @power-plant/bindings
```
...
```bash
npm install -D @power-plant/bindings
```
...
```bash
yarn add -D @power-plant/bindings
```
...
Run `nx build bindings` to build the library.

## Running unit tests

Run `nx test bindings` to execute the unit tests via [Vitest](https://vitest.dev/).
