...
# Power Plant - GraphQL Codegen Generator

A Power Plant generator for using GraphQL Codegen to generate source code from a GraphQL schema.
...
```bash
pnpm add -D @power-plant/graphql-codegen
```
...
```bash
npm install -D @power-plant/graphql-codegen
```
...
```bash
yarn add -D @power-plant/graphql-codegen
```
...
Run `nx test graphql-codegen` to execute the unit tests via [Vitest](https://vitest.dev/).
