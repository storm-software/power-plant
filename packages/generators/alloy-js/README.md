...
# Power Plant - Alloy JS Generator

A Power Plant generator for using Alloy JS to generate source code.
...
```bash
pnpm add -D @power-plant/alloy-js
```
...
```bash
npm install -D @power-plant/alloy-js
```
...
```bash
yarn add -D @power-plant/alloy-js
```
...
Run `nx build alloy-js` to build the library.

## Running unit tests

Run `nx test alloy-js` to execute the unit tests via [Vitest](https://vitest.dev/).
