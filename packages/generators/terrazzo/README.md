...
# Power Plant - Terrazzo Generator

A Power Plant generator for using [Terrazzo](https://terrazzo.app/docs/) to generate platform code from Design Tokens Community Group (DTCG) tokens.
...
```bash
pnpm add -D @power-plant/terrazzo
```
...
```bash
npm install -D @power-plant/terrazzo
```
...
```bash
yarn add -D @power-plant/terrazzo
```
...
Run `nx build terrazzo` to build the library.

## Running unit tests

Run `nx test terrazzo` to execute the unit tests via [Vitest](https://vitest.dev/).
