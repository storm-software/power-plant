...
# Power Plant - Hey API Generator

A Power Plant generator for writing the Hey API generated code to a directory.
...
```bash
pnpm add -D @power-plant/hey-api
```
...
```bash
npm install -D @power-plant/hey-api
```
...
```bash
yarn add -D @power-plant/hey-api
```
...
Run `nx build hey-api` to build the library.

## Running unit tests

Run `nx test hey-api` to execute the unit tests via [Vitest](https://vitest.dev/).
