...
# Power Plant - Handlebars Generator

A Power Plant generator for using Handlebars to generate source code.
...
```bash
pnpm add -D @power-plant/handlebars
```
...
```bash
npm install -D @power-plant/handlebars
```
...
```bash
yarn add -D @power-plant/handlebars
```
...
Run `nx build handlebars` to build the library.

## Running unit tests

Run `nx test handlebars` to execute the unit tests via [Vitest](https://vitest.dev/).
