...
# Power Plant - AsyncAPI Generator

A Power Plant generator for using @asyncapi/generator to generate code from an AsyncAPI schema.
...
```bash
pnpm add -D @power-plant/asyncapi
```
...
```bash
npm install -D @power-plant/asyncapi
```
...
```bash
yarn add -D @power-plant/asyncapi
```
...
Run `nx test asyncapi` to execute the unit tests via [Vitest](https://vitest.dev/).
