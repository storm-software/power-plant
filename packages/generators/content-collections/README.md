...
# Power Plant - Content Collections Generator

A Power Plant generator for using [Content Collections](https://www.content-collections.dev) (`@content-collections/core`) to validate content files against a provided schema and generate type-safe data access hooks (`allPosts`, …).
...
```bash
pnpm add -D @power-plant/content-collections
```
...
```bash
npm install -D @power-plant/content-collections
```
...
```bash
yarn add -D @power-plant/content-collections
```
...
Run `nx build content-collections` to build the library.

## Running unit tests

Run `nx test content-collections` to execute the unit tests via [Vitest](https://vitest.dev/).
